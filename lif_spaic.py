...
def compile_to_darwin():
    ...
    pops_data = {}
    pops_data['layer1'] = {}
    pops_data['layer1']['core_config'] = spaic_lif_config(vth=layer1_vth)


    pops_data_config = PopsDataConfig()
    pops_data_config.set_pops_data(pops_data) # 注册

    dump_pop(pop_list=layer1_neurons, pop_name='layer1',output_dir='./save_darwin_lif') # Todo  这个要怎么设定
    dump_input_neuron(input_pops=input0_neurons, output_dir='./save_darwin_lif')
    dump_output_neuron(output_pops=layer1_neurons, pop_name="output_pop", output_dir='./save_darwin_lif')
