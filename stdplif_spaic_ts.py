...
def compile_to_darwin():
    ...
    pops_data = {}
    pops_data['layer1'] = {}
    pops_data['layer1']['core_config'] = spaic_stdpexlif_ts_config(timestep=3) # 传两个负数一个0 ，再传 一个 0 的话 看看 电压是不是重置的
    pops_data['layer1']['vth_theta'] = (np.zeros((2,)) + 0) # 所有的vth_theta最开始都是0， 每次脉冲增加 0.5
    pops_data['layer1']['my_vth'] = (np.zeros(2,) - 52) # 初始化是-52
    pops_data['layer1']['my_loop_index'] = (np.zeros(2,)) # 初始化是 0 
